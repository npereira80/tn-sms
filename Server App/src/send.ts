...
export function enqueueSend(ctx: UserContext, requestedBy: string, to: string, body: string, attachments: SendAttachment[] = []) {
  const id = nanoid();
  const ts = now();
  const primary = currentPrimary(ctx);
...
export function updateSendStatus(ctx: UserContext, requestId: string, status: string) {
  ctx.db.prepare(`UPDATE send_request SET status = ?, updated_at = ? WHERE id = ?`)
    .run(status, now(), requestId);
  hub.broadcast(ctx.userId, { type: "send_status", requestId, status });
}

/** Re-dispatch queued sends to the primary when an agent (re)connects. */
export function redeliverQueued(ctx: UserContext, deviceId: string) {
  const rows = ctx.db
